...
已配置 Meting API 的主机自动受信任；音频 CDN、下载地址等额外主机可在 `.env` 的 `MUSIC_EXTERNAL_HOSTS` 中配置。所有音源、重定向和下载地址仍只能使用受信任的公网主机。

搜索时系统按优先级尝试支持该平台的 API。网络错误、超时、无效业务响应或空结果会自动尝试下一个可用 API。相同平台、关键词和页码的成功搜索结果会在应用进程内缓存 60 分钟（最多 400 条），并合并同一时刻的重复搜索，避免对上游 API、音频地址解析和时长检测造成重复负担。修改、禁用或删除音源配置会立即清空该缓存。可先使用“测试 API”检查每个平台能否正常搜索。
